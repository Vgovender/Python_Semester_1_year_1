...
def get_robot_name():
    """[gets the name of he robot from user]
...
def valid_command(command):
    ...
    if command_name.lower() == 'replay':
        if len(arg1.strip()) == 0:
            return True
        elif (arg1.lower().find('silent') > -1 or arg1.lower().find('reversed')\
            > -1) and len(arg1.lower().replace('silent', '').replace('reversed'\
            ,'').strip()) == 0:
            return True
        else:
...
def get_commands_history(reverse, relativeStart, relativeEnd):
    ...
    commands_to_replay = [(name, args) for (name, args) in list(map(lambda \
    command: split_command_input(command),history)) if name in move_commands]
    if reverse:
        commands_to_replay.reverse()

    range_start = len(commands_to_replay) + relativeStart\
    if (relativeStart is not None and\
    (len(commands_to_replay) + relativeStart) >= 0) else 0

    range_end = len(commands_to_replay) + relativeEnd if  (relativeEnd is not\
    None and (len(commands_to_replay) + relativeEnd) >= 0 and\
...
def call_command(command_name, command_arg, robot_name):
    """[redircts to the correct function based on command form user]

    Returns:
        [False]: [tells the robot to continue]
    """
    # help
...
def robot_start():
    ...
    if 'turtle' in sys.argv:
        world.turtle.world.draw_border()
        world.obstacles.generate_random_obstacles()
        lis = world.obstacles.get_obstacles()
        world.turtle.world.draw_obstacles(lis)
        ...
        if 'turtle' not in sys.argv:
            if lis != []:
                ...
                for i in lis:
                    print('- At position '+str(i[0])+','+str(i[1])\
                +' (to '+str(i[0]+4)+','+str(i[1]+4)+')')
